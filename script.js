...
function createTicket(task,priorityColor,ticketId){
    ...
    ticketCont.innerHTML = `<div class="ticket-color ${priorityColor}"></div> 
                            <div class="ticket-id">#gir${id}</div>
    ...
    ticketColor.addEventListener( "click", () => {
        ...
        let curColor = ticketColor.classList[1];
        // let idx;
        ...
        //     }
        // }
        let idx = color.findIndex( (col) => {
            return col == curColor;
        } );

        let nextInd = (++idx) % color.length;
        let nextColor = color[nextInd];
        ticketColor.classList.remove(curColor);
